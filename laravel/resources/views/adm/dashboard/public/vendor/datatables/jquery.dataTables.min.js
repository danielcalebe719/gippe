...
(function (h) {
	...
	function Z(a) {
		...
		h.each(a, function (e) {
			if ((b = e.match(/^([^A-Z]+?)([A-Z])/)) && -1 !== "a aa ai ao as b fn i m o s ".indexOf(b[1] + " "))
				(c = e.replace(b[0], b[2].toLowerCase())), (d[c] = e), "o" === b[1] && Z(a[e]);
		});
		a._hungarianMap = d;
	}
	function J(a, b, c) {
			...
			G = function (a) {
				if (c === k || c === i)
					(j = l[i]),
						...
						j && j._bAttrSrc
							? (N(j.mData._)(d, m), t(j.mData.sort, a), t(j.mData.type, a), t(j.mData.filter, a))
							: q
		...
		for (c = l.length; b < c; b++)
			(f = l[b]),
				...
				a.oFeatures.bSort &&
					...
					!1 !== f.bSortable &&
						(d.attr("tabindex", a.iTabIndex).attr("aria-controls", a.sTableId), Ma(a, f.nTh, b))),
			...
			if (0 !== i.length) {
				...
				(c = f.sZeroRecords),
					...
					(b[0] = h("<tr/>", { class: e ? d[0] : "" }).append(
						h("<td />", { valign: "top", colSpan: V(a), class: a.oClasses.sRowEmpty }).html(c)
					)[0]);
		...
		for (var f = a.sDom.split(""), g, j, i, m, l, q, k = 0; k < f.length; k++) {
			...
			if ("<" == j) {
				...
				if ("'" == m || '"' == m) {
					...
					-1 != l.indexOf(".")
						? ((m = l.split(".")), (i.id = m[0].substr(1, m[0].length - 1)), (i.className = m[1]))
						: "#" == l.charAt(0)
	...
	function vb(a) {
		...
		for (g = 0; g < c; g++)
			(m = b[g]),
				...
				t("mDataProp_" + g, i),
				d.bFilter &&
					(t("sSearch_" + g, l.sSearch), t("bRegex_" + g, l.bRegex), t("bSearchable_" + g, m.bSearchable)),
	...
	function qb(a) {
		var b = a.oClasses,
			...
			f = function () {
				...
				b != e.sSearch &&
					(ga(a, { sSearch: b, bRegex: e.bRegex, bSmart: e.bSmart, bCaseInsensitive: e.bCaseInsensitive }),
					(a._iDisplayStart = 0),
					P(a));
			},
	...
	function la(a) {
		...
		if (a.scrollBarVis !== L && a.scrollBarVis !== k) (a.scrollBarVis = L), $(a);
			...
			if ("" === c) {
				r.width = "100%";
				if (U && (p.find("tbody").height() > j.offsetHeight || "scroll" == l.css("overflow-y")))
	...
	function Fa(a) {
		...
		if (d || (!t && !f && !e && j == V(a) && j == m.length))
			...
			for (n = 0; n < i.length; n++)
				(o = c[i[n]]),
					(m[n].style.width = null !== o.sWidthOrig && "" !== o.sWidthOrig ? v(o.sWidthOrig) : ""),
					o.sWidthOrig &&
						f &&
						h(m[n]).append(
			...
			for (n = e = 0; n < i.length; n++)
				(k = h(m[n])),
					...
					(k = p.bBounding ? Math.ceil(m[n].getBoundingClientRect().width) : k.outerWidth()),
					(e += k),
					(c[i[n]].sWidth = v(k - g));
			b.style.width = v(e);
			o.remove();
	...
	function K(a, b, c, d) {
		...
		else if (((b = n.ext), (b = b.sErrMode || b.errMode), a && r(a, null, "error", [a, d, c]), "alert" == b))
			alert(c);
		else {
			...
			for (j = l.length; g < j; g++) {
				...
				if ("table" === b) (f = c.call(r, l[g], g)), f !== k && e.push(f);
					...
					for (m = o.length; h < m; h++)
						(f = o[h]),
							(f = "cell" === b ? c.call(r, l[g], f.row, f.column, g, h) : c.call(r, l[g], f, g, h, n)),
							f !== k && e.push(f);
				}
			}
			return e.length || d
		...
		var b = this.iterator(
				...
				function (b) {
					...
					for (g = a.length; f < g; f++)
						(c = a[f]),
							c.nodeName && "TR" === c.nodeName.toUpperCase() ? h.push(na(b, c)[0]) : h.push(O(b, c));
					return h;
				},
				1
			),
	...
	n.defaults = {
		...
		oLanguage: {
			...
			sEmptyTable: "Sem cadastros disponíves na tabela",
